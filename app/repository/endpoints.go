...
func (r *Repo) AddUser(user entities.User) (int, error) {
	...
	if err := r.DB.QueryRow(SQL, user.Login, user.Password, user.Age, user.UserRole).Scan(&id); err != nil {
		var pqErr *pq.Error
...
func (r *Repo) GetUser(login, password string) (entities.User, error) {
	...
	if err := r.DB.QueryRow(SQL, login, password).Scan(&user.ID, &user.UserRole); err != nil {
...
func (r *Repo) GetDirectorId(film entities.Film) (int, error) {
	...
	if err := r.DB.QueryRow(SQL, film.DirectorName).Scan(&id); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *Repo) GetFilmById(id int) (entities.FilmResponse, error) {
	...
	if err := r.DB.QueryRow(SQL, id).Scan(&film.ID, &film.Name, &film.Genre, &film.DirectorName, &film.Rate, &film.Year, &film.Minutes); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *Repo) GetAllFilms(SQL string) ([]entities.FilmResponse, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&film.ID, &film.Name, &film.Genre, &film.DirectorName, &film.Rate, &film.Year, &film.Minutes); err != nil {
